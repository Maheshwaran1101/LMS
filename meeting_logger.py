...
class MeetingLogger:
    ...
    def save_session(self, transcript, summary, segments, metadata):
    ...
    def get_analytics(self):
        ...
        for session in sessions:
            data = self.load_session(session["session_id"])

            transcript = data.get("transcript", {}).get("full_text", "")
            total_words += len(transcript.split())

            duration = data.get("metadata", {}).get("duration", "0s")

            try:
                total_duration += int(duration.replace("s", ""))
            except:
                pass
